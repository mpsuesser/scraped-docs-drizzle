---
url: https://orm.drizzle.team/docs/singlestore/sql
title: "Sql"
description: ""
...
Additionally, any dynamic parameters such as `${id}` will be mapped to the `?` placeholder, and the corresponding values will be moved to an array of values that are passed separately to the database.
...
For more detailed information about partial select queries, you can refer to the Core API documentation available at **[Core API docs](select.md#basic-and-partial-select)**.

**Select different custom fields from table**

Here you can see a usage for **[`sql<T>`](sql.md#sqlt)**, **[`sql``.mapWith()`](sql.md#sqlmapwith)**, **[`sql``.as<T>()`](sql.md#sqlast)**.
