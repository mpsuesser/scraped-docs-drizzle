---
url: https://orm.drizzle.team/docs/singlestore/seed-overview
title: "Seed Overview"
description: ""
...
**Example 4**: Seed the `users` table with 5 entities and populate the database with 100 `posts` without connecting them to the `users` entities. Refine `id` generation for `users` so that it will give any int from `10000` to `20000` and remains unique, and refine `posts` to retrieve values from a self-defined array
...
There are many more possibilities that we will define in these docs, but for now, you can explore a few sections in this documentation. Check the [Generators](seed-functions.md) section to get familiar with all the available generator functions you can use.
