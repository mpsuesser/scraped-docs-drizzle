...
`cast(... as int)` is necessary because `count()` returns `bigint` / `string` in PostgreSQL. Alternatively, you can use [`.mapWith(Number)`](sql.md#sqlmapwith) to cast the value to a number at runtime.
