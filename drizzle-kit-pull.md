...
Mostly all drivers of the same dialect share the same set of connection params, as for exceptions like `aws-data-api`, `pglite` and `d1-http` - you will have to explicitely specify `driver` param.
...
`drizzle-kit pull` will by default manage all tables in all schemas. You can configure list of tables, schemas and extensions via `tablesFilters`, `schemaFilter` and `extensionFilters` options.
...
| `driver` |  | Drivers exceptions `aws-data-api` `d1-http` `pglite` |
