...
Mostly all drivers of the same dialect share the same set of connection params, as for exceptions like `aws-data-api`, `pglite` and `d1-http` - you will have to explicitly specify `driver` param.
...
`drizzle-kit push` will manage all tables and schemas by default. You can configure list of tables, schemas and extensions via `tablesFilter`, `schemaFilter` and `extensionFilters` options.
