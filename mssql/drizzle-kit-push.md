---
url: https://orm.drizzle.team/docs/mssql/drizzle-kit-push
...
It’s the best approach for rapid prototyping and we’ve seen dozens of teams and solo developers successfully using it as a primary migrations flow in their production applications. It pairs exceptionally well with blue/green deployment strategy, managed database deployments, and CI/CD workflows.
...
`drizzle-kit push` will manage all tables and schemas by default. You can configure list of tables, schemas via `tablesFilter`, `schemaFilter` options.
...
npx drizzle-kit push dialect=mssql schema=src/schema.ts --tablesFilter=‘user*’ url=mssql://user:password@host:port/dbname
```
